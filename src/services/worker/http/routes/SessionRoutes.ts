  ...
  private async startGeneratorWithProvider(
    ...
    generatorPromise = agent.startSession(session, this.workerService)
      ...
      .finally(async () => {
        ...
        // are capped per session; any non-transport exit resets the count. A response
        // stall has its own bounded resume below, so it is excluded here.
        const isTransport = (reason ?? '').startsWith('transport') && reason !== 'transport:response_stall';
